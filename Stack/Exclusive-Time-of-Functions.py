def exclusiveTime(n: int, logs: list[str]) -> list[int]:
    ...
    Task
    ----------
    On a single-threaded CPU, we execute a program containing n functions. Each function has a unique ID between 0 and
    n-1.
    ...
    function is called twice, one call executing for 2 time units and another call executing for 1 time unit, the
    ...
    So function 0 spends 2 + 4 + 1 = 7 units of total time executing, and function 1 spends 1 unit of total time
    executing.
    """
    ans = [0] * n
    stack = []
    prev_t = 0
    for log in logs:
        op_id, op_type, op_t = log.split(':')
        op_id, op_t = int(op_id), int(op_t)
        if op_type == 'start':
            if stack:
                ans[stack[-1]] += op_t - prev_t
            stack.append(op_id)
            prev_t = op_t
        else:
            ans[stack.pop()] += op_t - prev_t + 1
            prev_t = op_t + 1
    return ans
