def sortJumbled(mapping: list[int], nums: list[int]) -> list[int]:
    ...
    Since 338 and 38 share the same mapped value, they should remain in the same relative order, so
    338 comes before 38.
    ...
    [123,456,789].
    """
    mp = {str(i): str(x) for x, i in zip(mapping, range(10))}
    nums.sort(key=lambda x: int(''.join([mp[s] for s in str(x)])))
    return nums
