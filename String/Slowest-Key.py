def slowestKey(releaseTimes: list[int], keysPressed: str) -> str:
    ...
    Keypress for 'c' had a duration of 49 - 29 = 20 (pressed at time 29 right after the release of
    the previous character and released at time 49).
    Keypress for 'd' had a duration of 50 - 49 = 1 (pressed at time 49 right after the release of
    the previous character and released at time 50).
    ...
    slowest_time = 0
    ans = ''
    for time, ch in zip(releaseTimes, keysPressed):
        rel_time = time - prev_time
        if rel_time > slowest_time:
            slowest_time = rel_time
            ans = ch
        elif rel_time == slowest_time:
            ans = max(ans, ch)
        prev_time = time
    return ans
